...
function b(l,k){var m=k&&k.loc,h=undefined,j=undefined;
if(m){h=m.start.line;
j=m.start.column;
l+=" - "+h+":"+j
}var i=Error.prototype.constructor.call(this,l);
for(var g=0;
g<f.length;
g++){this[f[g]]=i[f[g]]
}if(Error.captureStackTrace){Error.captureStackTrace(this,b)
}try{if(m){this.lineNumber=h;
if(c){Object.defineProperty(this,"column",{value:j,enumerable:true})
}else{this.column=j
}}}catch(n){}}b.prototype=new Error();
a["default"]=b;
d.exports=a["default"]
...
a["default"]=c;
b.exports=a["default"]
}),(function(b,r,e){var k=e(23)["default"];
var q=e(3)["default"];
var a=e(1)["default"];
r.__esModule=true;
r.checkRevision=l;
...
var c=a(m);
var n=e(4);
function l(v){var u=v&&v[0]||1,x=n.COMPILER_REVISION;
if(u!==x){if(u<x){var t=n.REVISION_CHANGES[x],w=n.REVISION_CHANGES[u];
...
}}}function p(D,x){if(!x){throw new c["default"]("No environment passed to template")
...
},nullContext:k({}),noop:x.VM.noop,compilerInfo:D.compiler};
...
var D=A;
if(A&&C!=A[0]&&!(C===t.nullContext&&A[0]===null)){D=[C].concat(A)
...
}function s(t,v,u){var x=u.data&&u.data["partial-block"];
u.partial=true;
if(u.ids){u.data.contextPath=u.ids[0]||u.data.contextPath
}var w=undefined;
if(u.fn&&u.fn!==f){(function(){u.data=n.createFrame(u.data);
var z=u.fn;
w=u.data["partial-block"]=function y(B){var A=arguments.length<=1||arguments[1]===undefined?{}:arguments[1];
A.data=n.createFrame(A.data);
A.data["partial-block"]=x;
return z(B,A)
};
if(z.partials){u.partials=d.extend({},u.partials,z.partials)
}})()
...
}function o(t,u){if(!u||!("root" in u)){u=u?n.createFrame(u):{};
u.root=t
}return u
...
if(W){for(var T in W){if(T in W){this.options.knownHelpers[T]=W[T]
...
}}}},compiler:c,compileChildren:function F(ax,aA){var av=ax.children,au=undefined,ar=undefined;
for(var ay=0,aw=av.length;
ay<aw;
ay++){au=av[ay];
ar=new this.compiler();
var at=this.matchExistingProgram(au);
if(at==null){this.context.programs.push("");
var az=this.context.programs.length;
au.index=az;
au.name="program"+az;
this.context.programs[az]=ar.compile(au,aA,this.context,!this.precompile);
this.context.decorators[az]=ar.decorators;
this.context.environments[az]=au;
this.useDepths=this.useDepths||ar.useDepths;
this.useBlockParams=this.useBlockParams||ar.useBlockParams;
au.useDepths=this.useDepths;
au.useBlockParams=this.useBlockParams
}else{au.index=at.index;
au.name="program"+at.index;
...
var ar=this.nameLookup("helpers",au,"helper"),ax=this.aliasable(this.contextName(0)+" != null ? "+this.contextName(0)+" : (container.nullContext || {})");
